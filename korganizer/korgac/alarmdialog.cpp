...
: KDialogBase(Plain, WType_TopLevel|WStyle_Customize|WStyle_StaysOnTop|WStyle_DialogBorder,
...
, mSuspendTimer(this)
{
  ...
  QLabel *label = new QLabel(i18n("The following events triggered reminders:"), topBox);
...
void AlarmDialog::slotUser2() {
  ...
  kapp->dcopClient()->send("korganizer", "KOrganizerIface", "editIncidence(QString)",
                           incidence->uid());
...
void AlarmDialog::slotSave() {
  ...
  if(lock.data()->lock() != KLockFile::LockOK) { return; }
...
QValueList< AlarmListItem * > AlarmDialog::selectedItems() const {
  QValueList<AlarmListItem*> list;
  for(QListViewItemIterator it(mIncidenceListView) ; it.current() ; ++it) {
    if(it.current()->isSelected()) { list.append(static_cast<AlarmListItem*>(it.current())); }
  }
  return list;
}
...
void AlarmDialog::suspendAll() {
  mIncidenceListView->clearSelection();
  for(QListViewItemIterator it(mIncidenceListView) ; it.current() ; ++it) {
    if(it.current()->isVisible()) { it.current()->setSelected(true); }
  }
  suspend();
}
